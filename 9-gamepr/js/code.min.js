...
(function() { var game = new Phaser.Game(980, 550, Phaser.AUTO, "game"); var login = { preload: preload, create: createLogin, update: updateLogin }; var menu = { preload: preload, create: createMenu }; var howToPlay = { preload: preload, create: createHowToPlay, update: updateHowToPlay }; var credit = { preload: preload, create: createCredit }; var report = { preload: preload, create: createReport, update: updateReport }; var gameplay = { preload: preload, create: createGameplay, update: updateGameplay }; var resultGame = { preload: preload, create: createResult };
    ...
    function preload() { game.load.spritesheet('enemyShip', 'images/enemyship.png', 600, 400);
        ...
        game.load.spritesheet('kraken', 'images/boss.png', 1191, 842);
    ...
    function createGameplay() { game.world.setBounds(0, 0, 2440, 550);
        music.stop();
        ...
        bombGroup.physicsBodyType = Phaser.Physics.ARCADE; for (var i = 0; i < 16; i++) { bomb = bombGroup.create(0, 0, 'boom');
        ...
        sharkGroup.physicsBodyType = Phaser.Physics.ARCADE; for (var i = 0; i < 16; i++) { shark = sharkGroup.create(0, 0, 'shark');
            shark.exists = false;
            shark.visible = false;
        ...
        textMute = game.add.button(900, 20, 'mute', function() { isSound = !isSound; if (!isSound) { game.sound.mute = true;
                textMute.frame = 1; } else { textMute.frame = 0;
                game.sound.mute = false; } });
        textMute.scale.setTo(0.1, 0.1);
        textMute.fixedToCamera = true; if (isSound) textMute.frame = 0;
        else textMute.frame = 1; } var fc = { apiKey: "AIzaSyAhBOG3nZFT0xjOu5UPm1k-ZVot1IPEfoQ", authDomain: "wip-camps-game.firebaseapp.com", databaseURL: "https://wip-camps-game.firebaseio.com", storageBucket: "wip-camps-game.appspot.com", messagingSenderId: "768785136426" };

    function updateGameplay() { if (isSound) { game.sound.mute = false; } else { game.sound.mute = true; }
        ...
        boomGroup.animations.play('move'); if (bombTime <= 0) { bombSpawn(); } if (cannonTime <= 0) { shootThem(); } if (sharkTime <= 0) { sharkSpawn(); }
        cannonTime--;
        bombTime--;
        sharkTime--; if (time % 720 == 480) warnning.animations.play('play'); if (time % 720 == 0) { warnning.animations.stop('play');
            warnning.frame = 0; } if (time % 720 >= 480 && time % 720 < 600) { if (!warnSound.isPlaying) { warnSound.play(); } } if (time % 720 <= 600) { playership.body.velocity.x -= 0.5; } else { playership.body.velocity.x -= 1.5; { if (!krakenSound.isPlaying) { krakenSound.play(); if (warnSound.isPlaying) { warnSound.stop(); } if (krakenSound.isPlaying) music.volume = 0.2; } else { sharkSound.volume = 0.6;
                    bombAwaySound.volume = 0.6;
                    bombDropSound.volume = 0.6;
                    buttonPush.volume = 0.6; } } }
        time++; if (playership.body.velocity.x <= maxSpeed) { if (cursors.right.isDown) { ck = 1;
                buttonPush = game.add.audio('buttonPush'); } if (ck == 1) { if (cursors.left.isDown && (!cursors.right.isDown)) { ck = 2;
                    buttonPush.play();
                    sharkSound.volume = 1;
                    bombAwaySound.volume = 1;
                    bombDropSound.volume = 1;
                    buttonPush.volume = 1;
                    music.volume = 1; } } if (ck == 2) { ck = 0;
                s += 5;
                playership.body.velocity.x += 20; } } if (playership.x < 250 + 550) { game.state.start('result');
            allSoundStopFx();
            death = game.add.audio('dKraken'); if (isSound) death.play(); } if (playership.x > game.world.width * (6.75 / 7) - 50) { game.state.start('result'); } if (jump.isDown && playership.body.touching.down) { playership.body.velocity.y = -500;
    ...
    function cannonHitPlayer() { allSoundStopFx();
        death = game.add.audio('dBomb');
        death.play();
        game.state.start('result'); }

    function bombHitPlayer() { allSoundStopFx();
        death = game.add.audio('dBomb');
        death.play();
        game.state.start('result'); }

    function ss() { var etk = dbEtk.child(name);
        e.on('value', function(snapshot) { try { hs = snapshot.val().hs; } catch (err) {} }); if (hs <= s || hs === undefined) { d.child(name).update({ "score": s, "highscore": s }); } else { d.child(name).update({ "score": s }); } }

    function sharkHitPlayer() { allSoundStopFx();
        death = game.add.audio('dKraken');
        death.play();
        game.state.start('result'); }

    function bombSpawn() { if (bombTimer < game.time.now) { var output = game.rnd.integerInRange(0, 20); if (!bombAwaySound.isPlaying) { bombAwaySound.play(); } if (output == 0) { bombTimer = game.time.now + 5000;
    ...
    function sharkSpawn() { if (sharkTimer < game.time.now) { var output = game.rnd.integerInRange(0, 20);
            sharkSound.play(); if (!sharkSound.isPlaying) { sharkSound.play(); } if (output == 0) { sharkTimer = game.time.now + 8000;
    ...
    function sharkKill(bomby) { bomby.animations.stop();
        bomby.kill(); }

    function shootThem() { if (cannonTimer < game.time.now) { var output = game.rnd.integerInRange(0, 20); { bombDropSound.play(); if (bombDropSound.isPlaying) { bombDropSound.restart(); } } if (output == 0) { enemyAnimations.play(5, false);
                cannonTimer = game.time.now + 12000; var range = game.rnd.integerInRange(-200, 200);
                bullet = cannonGroup.getFirstExists(false);
                bullet.reset(playership.x + range, 0);
                bullet.body.velocity.y = 100; } } }

    function createMenu() { bg = game.add.image(0, 0, 'bgStart');
        music.stop();
        ...
        textMute = game.add.button(900, 20, 'mute', function() { isSound = !isSound; if (!isSound) { game.sound.mute = true;
                textMute.frame = 1; } else { textMute.frame = 0;
                game.sound.mute = false; } });
        textMute.scale.setTo(0.1, 0.1); if (isSound) textMute.frame = 0;
        else textMute.frame = 1; }

    function createHowToPlay() { checkMove = 0;
    ...
    function updateHowToPlay() { game.physics.arcade.collide(playership, floor);
        game.physics.arcade.collide(playershipEx, floor); }

    function sendReportMessage(report) { var rndText = ""; var possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"; for (var i = 0; i < 5; i++) { rndText += possible.charAt(Math.floor(Math.random() * possible.length)); } var rnd = game.rnd.integerInRange(0, 1000000) + rndText + (new Date()).getTime(); var dbrp = firebase.database().ref().child("etk"); var etk = dbrp.child('report').child(rnd);
        etk.update({ 'message': report }); }

    function Jump() { playership.body.velocity.y = -500;
        playershipJump.play(3, false);
        animJumpSpacebar.play(2, false); }

    function shipMove() { if (checkMove % 2 == 0) { left.animations.play('do');
            right.animations.play('do');
            playershipEx.body.velocity.x = 70; } else { left.animations.stop('do');
    ...
    function createResult() { allSoundStopFx();
    ...
    function createReport() { ckReport = false;
        game.add.image(0, 0, 'bgHowToPlay');
        oldMap = game.add.image(980 / 2, 550 / 2, 'oldMap');
        oldMap.anchor.set(0.5);
        oldMap.scale.setTo(0.22, 0.22);
        ...
        input = game.add.inputField(980 * 0.5 / 4, 550 / 4, { font: '22px Thaisans Neue for Web', fill: '#212121', fontWeight: 'normal', width: 735, height: 250, borderColor: '#000', textAlign: 'left', padding: 10, max: 300, placeHolder: 'แจ้งข้อผิดพลาดได้ที่นี่เลยนะ :)' });
        input.startFocus();
    ...
    function updateReport() { if (ckReport) debugReport();
        window.input = input; } var inputDebug;

    function sendReport() { if (input.value != "") { isFirstSubmit = false;
            ckReport = true;
            buttonSound();
    ...
    function debugReport() { if (input.value != "") { isFirstSubmit = false;
            ckReport = false;
            buttonSound();
            sendReportMessage(input.value);
            input.value = null;
    ...
    function getHighScore() { var e = d.child(name);
        e.on('value', function(snapshot) { try { hs = snapshot.val().hs; } catch (err) {} }); if (hs === undefined) { hs = 0.0; } return highscore; }

    function createLogin() { isSound = true;
        random = 0;
        ...
        game.add.plugin(PhaserInput.Plugin);
        input = game.add.inputField(980 / 2 - 150, game.world.centerY - 12.5 + 50, { font: '22px Thaisans Neue for Web', fill: '#212121', fontWeight: 'normal', width: 300, height: 25, borderColor: '#000', borderRadius: 10, textAlign: 'center', padding: 10, max: 15, placeHolder: 'ใส่ชื่อของเจ้าที่นี่' });
    ...
    function createCredit() { game.add.image(0, 0, 'bgHowToPlay');
        oldMap = game.add.image(980 / 2, 550 / 2, 'oldMap');
        oldMap.anchor.set(0.5);
        oldMap.scale.setTo(0.22, 0.22);
    ...
    function toReport() { buttonSound();
        isFirstSubmit = true;
        game.state.start('report'); }

    function toGame() { buttonSound();
        game.state.start('gameplay'); }

    function tomenu() { game.state.start('menu'); }

    function toMenu() { if (input.value != "") { buttonSound();
            game.state.start('menu');
            name = input.value; } else { if (random == 0) message.text = "บอกชื่อของเจ้ามาก่อน";
            else if (random == 1) message.text = "นี่ถามไม่ตอบเรอะ !!";
            else if (random == 2) message.text = "เจ้าควรตอบคำถามข้านะ";
            else if (random == 3) message.text = "ยัง ยังอีก";
            else if (random == 4) message.text = "เจ้าทำให้ข้าหงุดหงิดนะ";
            else if (random == 5) message.text = "ข้าไหว้ละ";
            else if (random == 6) message.text = "บอกข้าเถอะข้าขอร้อง";
            else if (random == 7) message.text = "นร้า นร้า ๆ";
            else if (random == 8) message.text = "ถ้าไม่บอกข้าจะงอนแล้วนะ -*-";
            else if (random == 9) message.text = "เชอะ! งอนแล้ว";
            else if (random == 10) window.close();
            random++; } }

    function toHowToPlay() { buttonSound();
        game.state.start('howToPlay'); }

    function toCredit() { buttonSound();
        game.state.start('credit'); }

    function toLink() { window.open(linkToScore); }

    function buttonSound() { soundFx1 = game.add.audio('button');
        soundFx1.play(); }

    function allSoundStopFx() { if (sharkSound.isPlaying) { sharkSound.stop(); } if (bombAwaySound.isPlaying) { bombAwaySound.stop(); } if (bombDropSound.isPlaying) { bombDropSound.stop(); } if (krakenSound.isPlaying) { krakenSound.stop(); } if (warnSound.isPlaying) { warnSound.stop(); } }
